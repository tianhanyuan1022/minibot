...
异步业务代码应依赖 :class:`MemoryBackend`，本地部署使用
:class:`FileMemoryBackend`。旧调用点仍可暂时使用同步 :class:`MemoryStore`；
两者共享同一套加锁和原子写实现。
"""
...
def _namespace_dir(root: Path, namespace: str | None) -> Path:
    """默认 namespace 直接使用旧目录；命名空间只能位于根目录之下。"""
...
def _append_history(directory: Path, summary: str, kind: str) -> dict[str, Any]:
    ...
    with _directory_lock(directory):
        # 逻辑上仍是 append-only；整文件 replace 避免并发追加出现半行。
...
def _load_bundled_template(name: str) -> str | None:
    """从包里的 `templates/` 目录读取默认内容（比如 `SOUL.md`）。

    对应 nanobot 的 `load_bundled_template()`（`nanobot/utils/helpers.py`）：
    默认内容作为真正的 `.md` 文件随包分发，而不是写成 Python 字符串常量。
    这类内容不是"喂给模型的 system prompt"（那部分见 `prompts.py`），而是给
    *人* 看、给人编辑的初始文档，用 `.md` 文件天然更合适：语法高亮、方便 diff、
    不需要转义引号/换行。

    用 ``suppress(Exception)`` 兜底：模板读取失败时退化到最小占位内容。
    """
    with suppress(Exception):
        tpl = resources.files("mini_nanobot") / "templates" / name
        if tpl.is_file():
